...
class CapabilityProbe:
    def run(self) -> CapabilityReport:
        ...
        gpu_info, gpu_issues = self._probe_gpu(platform_info)
        docker_info, docker_issues = self._probe_docker(platform_info)
        capabilities = self._derive_capabilities(platform_info, gpu_info, docker_info)
        recommendation = self._recommend(platform_info, gpu_info, capabilities)
    ...
    def _probe_gpu(self, platform_info: PlatformInfo) -> tuple[GpuInfo, list[ProbeIssue]]:
        ...
        if amd_name:
            return GpuInfo(vendor="amd", model=amd_name.strip(), source="sysfs"), issues
        ...
        return GpuInfo(), issues

    def _probe_docker(
        self,
        platform_info: PlatformInfo,
    ) -> tuple[DockerCapability, list[ProbeIssue]]:
        issues: list[ProbeIssue] = []
        if not shutil.which("docker"):
            return DockerCapability(), issues
        ...
        try:
            ...
            return DockerCapability(available=True), issues

        runtime = payload.get("DefaultRuntime")
    ...
    def _derive_capabilities(
        ...
        docker_nvidia_gpu = (
            ...
            and gpu_info.vendor == "nvidia"
            and gpu_info.model is not None
            and platform_info.is_wsl
        )
        docker_vulkan = False
    ...
    def _recommend(
        ...
        reasons: list[str] = []
        if capabilities.docker_nvidia_gpu and gpu_info.vram_mb and gpu_info.vram_mb >= 4096:
...
def _safe_run(args: list[str]) -> subprocess.CompletedProcess[str]:
